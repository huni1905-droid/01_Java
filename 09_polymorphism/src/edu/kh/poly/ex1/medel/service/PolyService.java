package edu.kh.poly.ex1.medel.service;

import edu.kh.poly.ex1.medel.dto.Car;
import edu.kh.poly.ex1.medel.dto.Spark;
import edu.kh.poly.ex1.medel.dto.Tesla;
...
// 부모타입 참조변수 = 부모객체주소
		...
		// 부모타입 참조변수 = 자식객체주소
		Car car2 = new Tesla(); // 자료형이 다른데도 오류발생 안함
		...
		// 2) 다형성을 이용한 객체배열
		Car[] arr = new Car[3];
		// 각 요소가 Car 객체만 들어올 수 있는 객체배열을 선언 및 할당
		...
		// 다형성(업 캐스팅) : 부모타입(Car) 참조변수 arr[i]로 자식객체(Tesla, Spark)를 참조할 수 있다.
	...
	public void printCar(Car temp) {
		...
		// temp라는 Car타입(부모타입) 매개면수에 Car, Tesla, Spark의 
		// 주소가 참조가능하다!!!!
	...
	// 매개변수로 전달받은 정수갑에 따라
	// Car, Tesla, Spark 중 객체를 만들어 반환하는 메서드
	public Car createCar(int num) {
		//업캐스팅을 이용해서 타입을 Car로 해준다
	...
	public void ex2() {
		...
		// 객체가 어떤 타입(자료형)인지 검사
		// instnaceof 연산자 : 객체의 자료형을 검사하는 연산자
	...
	public void ex3() {
		...
		Tesla t1 = (Tesla)c1; // 다운캐스팅을 먼저 하고 프린트문 써도 된다 
	...
	public void ex4() {
		...
		// -> 실행 시 런타임에러 발생
		// ClassCastException : 형변환 예외(형변환 잘못하면 나타나는 에러)
	...
	public void ex5() {
		...
		// toString에 마우스 대면 : String edu.kh.poly.ex1.medel.dto.Car.toString()
		// 반환형 출처
		// Car 객체에 있는 toString() 메서드를 호출
		// 컴파일러는 toString() 메서드가 Car에 있는걸로 인식해서
		// c1.toString() 호출코드와
		// edu.kh.poly.ex1.medel.dto.Car.toString() 메서드 선언부 코드를 연결
		// -> 정적 바인딩
		
		
		// ** 다형성 적용 시 바인딩 **
		Car c2 = new Spark("경차엔지", "휘발유", 4, 0.5);
		...
		// -> "실행 시"에는 Spark에 오버라이딩된 toStirng()이
