...
public class StudentService {
	...
	// * 인터페이스는 객체 생성 불가, 단 부모 참조변수로서의 역할은 가능

	// java.util.ArrayList : 배열 형태의 List (가장 대표적인 List 자식 클래스)
	// -> 검색(조회) 효율적
	// java.util.Vector : 동기화 제공하여 List 중 성능이 가장 좋지 않음
	// cf) 동기화 : 줄 세우는것 -> 끝날 때 까지 다음 수행하지 않아
	//     비동기화 : 줄 세우지 않고 들어오는대로 받아주는 것 -> 빨라 but 실수일어날수 있어
	// java.util.LinkedList : 값을 추가, 수정, 삭제시 효율적
	// 양 옆에 애들까지 인접참조하기 때문에

	// <E> : 제네릭
	// 타입을 제한하는 문법
	// 제네릭은 객체(Reference Type, 참조형)만 허용
	// Java 7 버전 이상에서는 우변의 제네릭 타입을 생략 가능(다이아몬드 연산자 생겨서)
	// 다이아몬드 연산자 : 컴파일러가 좌변 부분을 보고 우변의 타입을 추론
	// (컴파일러가 List<Integer> 보고 ArrayList<>도 Integer라고 생각)
	...
	public void ex() {// List 테스트용 메서드
		...
		testList.add(100); // AutoBoxing(자동으로 기본자료형(int)을 객체형태(Integer)로 변환함)
		testList.add(new Object());

		// 컬렉션 특징 : 여러 타입의 데이터를 저장할 수 있다.
		// 여러 타입을 저장하고 싶으면 List의 제네릭을 Object로 쓰면 된다.
		...
		// 향상된 for문 for(하나씩 꺼내오는 타입 : 배열 또는 컬렉션)
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				// menuNum이 초기 값인 0을 가지고있어 반복문이 종료됨.
	...
	public String addStudent() {
		// CheckedException : 반드시 throws 구문이나 try-catch 처리 강제해야 한다
		// IOException, SQLException..

		// UncheckedException : 강제 처리하지 않아도 되는 애 -> 강제 처리X, 예외 발생하면
		 ...
		 * studentList.add(new Student(name, age, region, gender, score)); boolean
		 * result = studentList.add(std); if(result){
		 * 
		 * } else{
		 * 
		 * }
	...
	public void selectAll() {
		...
		// studuntList가 비어있는지 확인
		// List에 저장된 데이터의 개수를 얻어오는 방법1 : int List.size()
		// 방법2 : boolean List.isEmpty() : 컬렉션이 비어있다면 true 반환, 아니면 false
		
		// 방법1 -> if(studentList.size() == 0)
		// 방법2 -> 아래와 같이
		...
		// 방법1. 일반 for문
		/* 
		for(int i=0; i < studentList.size(); i++) {//List에서는 length가 아니라 size를 써줘야 한다
		...
		*/				
		...
		int index = 0; // 1번 홍길동 형식으로 만들기 위한 인덱스 번호
		for( Student std : studentList) {
		//타입 [임의의 이름] : 하나씩 꺼내올 리스트(컬렉션)	
			System.out.print( (index++) + "번 : ");
			System.out.println(std);
			// == System.out.println(std.toString());
			// System.out.print( (index++) + "번 : " + std);형식으로 하면 한줄에 학생 전체 다 나와서 좋지 않다
	...
	public String updateStudent() {
		...
		// E List.set(int index, E e(객체)) 
		//: 주어진 index번째의 주어진 객체로 덮어쓰기(수정함) & 기존의 객체를 반환함
	...
	public String removeStudent() {
		...
		//		 remove 동작 시 뒤쪽 요소를 한칸씩 자동으로 당겨오는 행위를 하고 있음
		if(ch == 'Y') {
	...
	private void sortByName() {
		...
		Collections.sort(studentList, new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				// 이름 비교
				// name은 String 형이라 comparetTo()로 비교
				// String.comparaTo() : 자바에서 객체를 비교하는 메소드.
				// (String이 Comparable을 상속받아 재정의해둔
				// compareTo() 메서드를 이용하는 것)
				// -> 두 객체를비교하고 문자열의 순서 결정함.
