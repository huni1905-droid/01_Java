...
public class MemberService {
	...
	public void displayMenu() {
		...
		do {
			...
			switch(menuNum) {
			// 내가 호출한 메서드가 returnm 하는 값을 잘 생각하기!(ex) int, String.....)
	...
	// 회원 가입 기능 (정의) -> 밑에 다 만들고 코드 수행하려면 메서드 호출을 해줘야한다.
	public String signUp() {
		
		System.out.println("\n **********회원 가입**********");
		
		System.out.print("아이디 : ");
		String meberId = sc.next();
		
		System.out.print("비밀번호 : ");
		String meberPw = sc.next();
		
		System.out.print("비밀번호 확인 : ");
		String meberPwCheck = sc.next();
		
		System.out.print("이름 : ");
		String meberName = sc.next();
		
		System.out.print("나이 : ");
		int meberAge = sc.nextInt();
		
		
		// 비밀번호와 비밀번호 확인이 일치하면 회원가입 진행
		// 일치하지 않으면 회원가입 실패
		if( meberPw.equals(meberPwCheck) ) { // 일치하는 경우
			
			// 회원 가입
			// 새로운 회원 객체가 생성
			// -> Member 클래스 이용해서 새로운 객체를 생성
			// -> 매개변수 생성자 이용
			memberInfo = new Member(meberId, meberPw, meberName, meberAge);
	...
	public String login() {
		...
		// 내가 입력한 아이디와ㅣ 비밀번호로 가입한 사람의 정보가 있는지 확인!
		...
		// 로그인 기능 수행
		System.out.print("아이디 : ");
		String memerId = sc.next();
		
		System.out.print("비밀번호 : ");
		String memerPw = sc.next();
		...
		if( memerId.equals(memberInfo.getMemberId()) && (memerPw.equals(memberInfo.getMemberPw())) ) {
			
			// 로그인한 사람의 정보를 기억시키기
			// loginMember에 현재 로그인한 사람의 정보를 기억시키기
			// == loginMember에 memberInofo가 참조하고있는 객체의 주소 대입
