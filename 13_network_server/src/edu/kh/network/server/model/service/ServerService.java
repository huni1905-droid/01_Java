...
public class ServerService {
	...
	// Socket은 이 연결의 끝점 역할을 하며, 데이터 송수신 통로가 됨
	...
	public void serverStart() {
		...
		//  데이터를 쉽게 출력 가능하게 해주는 보조스트림들
		
		try {
			// 2. 서버용 소켓 객체 생성
			serverSocket = new ServerSocket(port); // (스타벅스 문 열고 고객 기다리는것과 같다)
			...
			clientSocket = serverSocket.accept();// 클라이언트의 요청이 오면 수락하는 것
			...
			// 문자 기반 BufferedReader와 바이트 기반 InputStream을 연결 할 수 있도록 
			...
			// SimpleDateFormate : 날짜 형식을 간단히 지정하는 객체
			...
			pw.println(str); // 서버 -> 클라이언트 로 데이터를 출력(전송)
			pw.flush(); // flush() : 스트림 내용을 밀어냄 (출력에서만 꼭 써주기)
