...
public class MemberService {
	...
	public MemberService() { // 기본생성자 
		// memberArr 배열의 1,2,3, 인덱스 초기화(이미 저장된 멤버들 실행시키면 
		//											바로 나올수 있게 만들어준것)
	...
	public int emptyIndex() {
		...
		for(int i=0; i < memberArr.length; i++){
			...
			if(memberArr[i] == null) {
				return i;
				// 현재 메서드를종료하고 호출한 곳으로 i 값을 가지고 되돌아감
			}
		}
	...
	public String signUp() {
		...
		// -> 없을 때 : -1 반환 (인덱스는 절대 음수값이 없으므로 -값으로 정의)
		...
		for(int i =0; i < memberArr.length; i++) {
			...
			if(memberArr[i] != null) { // 회원 정보가 있을 경우
				...
				if(memberArr[i].getMemberId().equals(memberId )
					...
					break; // 더이상 같은 아이디와 비밀번호가 없을것이라고 예상
						   // 효율을 위해 for문 종료
				}
			}
		} // for문 종료
	...
	public String selectMember() {
		...
		// 1) 로그인 여부 확인
		// 필드 loginMember가 참조하고 있는 객체 있는지 확안
		if(loginMember == null) {
			return "로그인 후 이용바람";
		} 
		
		// 2) 로그인이 되어있는 경우
		// 현재 로그인한 회원의 정보를 출력할 문자열을 만들어서 반환한다.
	...
	public int updateMember() {
		...
		// 4) 비밀번호가 같을 경우
		//	로그인한 회원의 이름, 나이, 지역을 입력받은 값으로 변경 후 
		// 	1반환
	...
	public void searchRegion() {
		...
		String inutRegion = sc.next();
		
		
		// memberArr 배열의 모든 요소 순차 접근
		boolean flag = true; // 검색 결고 신호용 변수
		...
		for(int i =0; i < memberArr.length; i++) {
			...
			//	memberArr[i] 요소에 저장된 지역(region)이 입력받은 
			//지역(inputRegion)과 같을 경우 memberArr[i] 요소에 저장된 객체의 
			//회원아이디, 이름을 출력
			if(memberArr[i].getRegion().equals(inutRegion)) {
