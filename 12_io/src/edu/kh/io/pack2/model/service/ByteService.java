...
public class ByteService {
	 ...
	 *  + 데이터 출력
	 */
	public void filByteOutput() {
		
		// FileOutputStream 참조 변수 선언 (OutputStream 상속받은 자식 클래스)
		FileOutputStream fos = null;
		
		//io클래스에 있는 애들은 Checked Exception 이라서
		try {
			// FileOutputStream : 경로 상 파일을 찾을 수 없음
			...
			//예외 처리해줘야하는 부분이라 위에서 정의해주지 않고 try 안에서 객체 만들어주는것 
			...
			*/
			...
			fos.flush(); // FileOutputStream에서는 flush() 선택 사항 (but 어디든 꼭 쓴다고 생각)
			// flush : 스트림 안에 남아있는 모든 데이터를 외부로 모두 밀어냄
			
			System.out.println("출력 완료!");
			
		} catch (IOException e) {//FileOutputStream은 IOException의 자식이라서 IOException로 잡을 수 있다.
			e.printStackTrace();
			
		} finally { // try 예외 발생 여부와 관계 없이 무조건 수행 구간
	...
	public void fileByteInput() {
		...
		try {
			...
			// FileInputStream 이용 시 해당 경로에 반드시 실제 파일이 존재햐야함!
			// -> FileNotFoundException 발생시킴
			// : /io_test/20251014/노래가사.txt (지정된 파일을 찾을수 없습니다)
			...
			*/
			...
			// 방법2. 파일에 저장된 모든 byte값을 다 읽어와 byte[] 형태로 반환 받기
	...
	public void bufferedFileByteInput() {
		...
		try {
			...
			try {
				// 보조 스트림만 close() 해도 기반 스트림만 같이 close() 됨
	...
	public void fileCopy() {
		...
		try {
			...
			try {
				...
				if( br != null ) br.close();
				if( bis != null ) bis.close(); // 보조스트림을 닫으면 fis(기반스트림)도 닫힘
				if( bos != null ) bos.close(); // 보조스트림을 닫으면 fos(기반스트림)도 닫힘
