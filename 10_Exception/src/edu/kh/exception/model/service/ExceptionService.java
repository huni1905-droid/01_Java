...
public class ExceptionService {
	...
	public void ex1() {
		...
		// int 형이 아닌 다른 자료형을 대입했을 때(int에 가나다 입력)
		...
		//catch (Exception e) { // 처럼 써도 된다.
			// Exception e = new ArithmeticException 이라서 
			//(모든 Exception의 최고 조상이 Exception라서)Exception e 라고 써도 된다
			
			// try 에서 던져진(발생된) 예외를 catch문의 매개변수 e로 잡음.
			// e를 사용하여 예외 추적 코드를 작성할 수도 있다
	...
	public void ex2() {
		...
		//Exception과 같이 Java.lang에 있는 페키지는 import 안해줘도 쓸 수 있어
			System.out.println("0으로 나눌 수 없습니다!");
			
		}catch (RuntimeException e) { // 부모 Exception들은 순차적으로 아래에 써줘야 에러 발생 안한다.
			System.out.println("잡았다!!");
			
		} catch (Exception e) {
			// 다형성 - 업캐스팅 : 부모타입 참조변수로 자식객체 참조
			// -> 상위 타입(부모님 타입)의 예외 클래스를 catch문에 작성하면
	...
	public void ex3() {
		...
		try {
			...
			System.out.println(e.getClass());// getClass : 어떤 예외 클래스인가? 알려주는 메서드
			// class java.lang.Ari...
			
			System.out.println(e.getMessage()); // getMessage : 예외 발생시 출력된 내용 출력해주는 메서드
			// / by zero
			
			System.out.println( e ); // e.toString() 과 동일
			
			e.printStackTrace(); // 예외에대한 상세히 추적해주는 메서드
			
			
		} finally {
			
			System.out.println("무조건 수행됨");
			sc.close(); // 스캐너 통로 닫기 - 메모리 누수 방지위해서
			
			
			
		}
		
		// try-wth-resources : finally에서 일일이 close() 호출하지 않아도
		// 자동으로 리소스를 닫아주는 방식
